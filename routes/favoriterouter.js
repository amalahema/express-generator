...
.post(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
    ...
    .then((favorite) => 
    {
        ...
        {           
            ...
            {
                res.setHeader("Content-Type", "application/json");
                res.status(404).json({ message: "Your Favorite item Already exists in the List" });
                  
            }
        }
        
        //called when it is empty(when we add the 1st fav in the list)
        else {
            Favorites.create({"user": req.user._id, "dishes": [req.params.dishId]})  //IF ITS ALREADY IN THE LIST MEAN PATICULAR INFORMATION OF USER AND DISHES ARE ADDED AS A FAVORITES LIST
            .then((favorite) => {
                console.log('Favorite list starts up', favorite);
                res.statusCode = 200;
                res.setHeader('Content-Type', 'application/json');
                res.json(favorite);//Display information of user & dish id
